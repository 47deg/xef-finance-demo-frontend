    ...
    if (!loading) {
      try {
        ...
        if (aiResponse.FriendlyResponse.includes('XXX') && mainQueryResponse) {
          ...
          if (
            mainQueryResponse.tableResponse.columns[0] &&
            ...
            const possibleValue: string =
              ...
              !mainQueryResponse.tableResponse.columns[0].includes('count')
                ...
                : mainQueryResponse.tableResponse.rows[0][0]
            friendlyResponse = aiResponse.FriendlyResponse.replace(
              'XXX',
              possibleValue,
            )
          } else {
        ...
        if (
          ...
          aiResponse.DetailedResponse.length > 0
        ) {
          detailedQueryResponse = await GenericQuery(
            aiResponse.DetailedResponse,
          )
        }
